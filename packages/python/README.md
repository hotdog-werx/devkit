...
- [`toolbelt`](https://pypi.org/project/tbelt/) profile configs
  (`toolbelt/python.yaml`, `toolbelt/python-dev.yaml`) that batch
  format/check commands per file type.
...
metadata in the consumer's own `pyproject.toml`, then maps template resources
to destination paths in the consumer repo.

## Development

This package is part of the `devkit` uv workspace. See the repo root
`README.md` for shared workspace tooling.
