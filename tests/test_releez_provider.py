...
def test_render_all_succeeds_use_self_action_false():
    bed = ProviderTestBed(ReleezProvider, ReleezContext(repo='uv-toolbox', use_self_action=False))
...
def test_render_all_succeeds_use_self_action_true():
    bed = ProviderTestBed(ReleezProvider, ReleezContext(repo='releez', use_self_action=True))
    workflow = bed.render(FINALIZE_RELEASE_TEMPLATE)
...
def test_finalize_release_uses_mise_tasks_for_build_and_publish():
    bed = ProviderTestBed(ReleezProvider, ReleezContext(repo='example'))
    workflow = bed.render(FINALIZE_RELEASE_TEMPLATE)
...
def test_cliff_toml_is_valid_toml_and_preserves_tera_syntax():
    ...
    place.
    """
    bed = ProviderTestBed(ReleezProvider, ReleezContext(owner='hotdog-werx', repo='releez'))
...
def test_no_pyright_references():
    bed = ProviderTestBed(ReleezProvider, ReleezContext())
